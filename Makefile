...
build:
	go build $(LD_FLAGS) ./cmd/main.go

install:
	go install $(LD_FLAGS) ./cmd/main.go
...
DOCKER_TAGS=-t bonnefoa/channelz-proxy:latest \
	-t bonnefoa/channelz-proxy:$(shell git describe --tags)
